...
Tabs.Main:AddButton({ Title = "Fix Lag X1 | Booster", Description = "Xoá 10% Độ Họa", Callback = function() -- Xóa quần áo và phụ kiện để giảm lag 
for _, player in pairs(game:GetService("Players"):GetPlayers()) do if player.Character then for _, v in pairs(player.Character:GetDescendants()) do if v:IsA("Shirt") or v:IsA("Pants") or v:IsA("ShirtGraphic") or v:IsA("Accessory") then v:Destroy() end end end end
